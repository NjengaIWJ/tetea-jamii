...
export const login = async (req, res) => {
    ...
    try {
        ...
        const token = jwt.sign(payload, Buffer.from(authConfig.jwtSecret), {
...
export const refresh = async (req, res) => {
    try {
        ...
        let payload;
        try {
            payload = jwt.verify(token, Buffer.from(authConfig.jwtSecret));
        }
        catch (err) {
        ...
        const newToken = jwt.sign(newPayload, Buffer.from(authConfig.jwtSecret), { expiresIn: authConfig.jwtExpiry } // e.g. '15m' or '1h'
        );
