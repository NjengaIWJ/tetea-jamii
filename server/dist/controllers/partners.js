...
const createPartner = async (req, res) => {
    ...
    const image = req.file ?? (req.files && req.files[0]);
    ...
    if (process.env.NODE_ENV === 'development') {
        ...
        console.debug('createPartner: req.files=', req.files);
        console.debug('createPartner: req.file=', req.file);
    }
    try {
        ...
        let imageURL;
        try {
            const { secure_url, public_id, bytes } = await (0, cloudinary_1.streamUpload)(image);
            imageURL = secure_url;
            console.log(`Uploaded to Cloudinary: ${public_id} (${(bytes / 1024).toFixed(2)} KB)`);
            const newPartner = await Partner_model_1.default.create({
                name: name.trim(),
                media: imageURL,
                publicId: public_id
            });
...
const getPartners = async (req, res) => {
    try {
        ...
        const partners = await Partner_model_1.default.find().sort({ createdAt: -1 });
...
const updatePartner = async (req, res) => {
    ...
    try {
        ...
        if (req.file) {
            ...
            const { secure_url, public_id } = await (0, cloudinary_1.streamUpload)(req.file);
        ...
        const updatedPartner = await Partner_model_1.default.findByIdAndUpdate(id, // The document ID
...
const deletePartner = async (req, res) => {
    ...
    try {
        ...
        const deletedPartner = await Partner_model_1.default.findByIdAndDelete(id);
        if (!deletedPartner) {
            return res.status(404).json({ error: "Partner not found" });
        }
